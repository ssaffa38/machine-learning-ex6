...
% here are the values of C and sigma to be tested
C_trial = sigma_trial = [0.01 0.03 0.1 0.3 1 3 10 30];

% Here are the number of trials for C and sigma
m = size(C_trial, 2);

% Average number of wrong predictions, initialized to 1
missShot = 1;
...
for (i = 1:m)
	for (j = 1:m)
		...
		% If there is an error rate of prediction that is smaller than previous one, then set missShot to the new value
